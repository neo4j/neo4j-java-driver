...
abstract class AbstractBaseEnvelopeEncryptionIT<T extends BasePropertyEncryption> {
    ...
    void beforeEach() throws IOException, NoSuchAlgorithmException {
        ...
        var keyRepository = new Neo4jEncapsulatedKeyRecordRepository(keyDriver, "neo4j");
        var version = DockerImageName.parse(neo4jContainer.getDockerImageName()).getVersionPart();
        var versions = version.split("\\.");
        var major = Integer.parseInt(versions[0]);
        var legacySyntax = false;
        if (major < 4) {
            legacySyntax = true;
        } else if (major == 4) {
            var minor = Integer.parseInt(versions[1]);
            legacySyntax = minor < 4;
        }
        keyRepository.createConstraints(legacySyntax);
