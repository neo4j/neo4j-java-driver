...
public abstract class AbstractReactiveStreamsEnvelopeEncryptionIT
    ...
    protected BaseEncapsulatedKeyManager.EncapsulatedKey createKey(String keyAlias) {
        return Mono.fromDirect(keyManager.create(keyAlias)).block();
    }

    @Override
    protected void updateAliasById(String id, String alias) {
        Mono.from(keyManager.updateAliasById(id, alias)).block();
    }

    @Override
    protected void deleteAliasById(String id) {
        Mono.from(keyManager.deleteAliasById(id)).block();
    }

    @Override
    protected void deleteKey() {
        Mono.fromDirect(keyManager.findByAlias(keyAlias))
                .flatMap(key -> Mono.fromDirect(keyManager.deleteById(key.id())))
                .block();
    }
    ...
    protected Optional<BaseEncapsulatedKeyManager.EncapsulatedKey> findByAlias(String alias) {
        return Optional.ofNullable(Mono.from(keyManager.findByAlias(alias)).block());
    }
}
