...
final class Neo4JKeyRecordRepository implements EncapsulatedKeyRecordRepository {
    private final Driver driver;
    private final String database;

    Neo4JKeyRecordRepository(Driver driver, String database) {
    ...
    public CompletionStage<EncapsulatedKeyRecord> findByAlias(String alias) {
        return session().executeReadAsync(tx -> tx.runAsync(
                        "MATCH (key:Key) WHERE key.alias=$alias RETURN key", Map.of("alias", alias))
                .thenCompose(resultCursor -> resultCursor.nextAsync().thenApply(this::toKey)));
    }
    ...
    public CompletionStage<EncapsulatedKeyRecord> create(
        ...
        return session().executeWriteAsync(tx -> tx.runAsync(
                ...
                .thenApply(summary -> EncapsulatedKeyRecords.create(id, alias, encapsulation, metadata)));
    }

    @Override
    public CompletionStage<Void> setAliasById(String id, String alias) {
        return session().executeWriteAsync(tx -> {
            var params = Map.<String, Object>of("id", id, "alias", alias);

            var query = """
                MATCH (k:Key {id: $id})
                WITH k, k.alias AS previousAlias
                SET k.alias = $alias
    ...
    private EncapsulatedKeyRecord toKey(Record record) {
        ...
        return EncapsulatedKeyRecords.create(id, alias, encapsulation, Map.copyOf(managerMetadata));
    }
}
