...
public final class BoltConnectionImpl implements BoltConnection {
    ...
    public BoltConnectionImpl(
        ...
        var mappedRoutingContext = new HashMap<String, Value>();
        for (var entry : routingContext.toMap().entrySet()) {
            mappedRoutingContext.put(entry.getKey(), Values.value(entry.getValue()));
        }
        this.routingContext = Collections.unmodifiableMap(mappedRoutingContext);
    ...
    public CompletionStage<Void> close() {
        try {
            return connection.close().exceptionally(ignored -> null);
