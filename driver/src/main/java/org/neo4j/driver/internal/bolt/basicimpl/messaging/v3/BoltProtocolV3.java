...
import org.neo4j.driver.internal.bolt.routedimpl.cluster.ClusterComposition;
...
public class BoltProtocolV3 implements BoltProtocol {
    ...
    public void initializeChannel(
        ...
        if (routingContext.isServerRoutingEnabled()) {
            ...
            message = new HelloMessage(
                    userAgent, null, authMap, null, includeDateTimeUtcPatchInHello(), notificationConfig);
        }
    ...
    public CompletionStage<Void> route(
        ...
        var runHandler = new RunResponseHandler(runFuture, METADATA_EXTRACTOR, connection, null);
        ...
        runFuture
                ...
                .thenApply(ignored -> {
                    ...
                    for (var serversMap : map.get("servers").asList(MapAccessor::asMap)) {
                        ...
                        for (var server :
                            ...
                            if (role.equals("WRITE")) {
                                writers.add(address);
                            } else if (role.equals("READ")) {
                                readers.add(address);
                            } else if (role.equals("ROUTE")) {
                                routers.add(address);
                            }
                        }
                    }
                    ...
                    if (!routers.isEmpty() && !readers.isEmpty()) {
                        ...
                        // todo sync with the original error message
    ...
    public CompletionStage<Void> close(Connection connection) {
        var goodbyeHandler = new NoOpResponseHandler();
        connection.write(GoodbyeMessage.GOODBYE, goodbyeHandler).whenComplete((ignored, throwable) -> {
            if (throwable == null) {
                connection.flush();
            }
        });
