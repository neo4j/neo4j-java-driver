    ...
    public CompletionStage<Void> close() {
        if (closeFuture == null) {
            ...
            if (delegate.state() == BoltConnectionState.CLOSED) {
                ...
                return closeFuture;
            } else if (delegate.state() == BoltConnectionState.FAILURE) {
                purgeRunnable.run();
                return delegate.close();
            }

            var resetFuture = new CompletableFuture<>();
            delegate.reset()
                    .thenCompose(boltConnection -> boltConnection.flush(new ResponseHandler() {
                        @Override
                        public void onError(Throwable throwable) {
