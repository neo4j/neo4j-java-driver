...
public class RoutedBoltConnection implements BoltConnection {
    ...
    public CompletionStage<Void> flush(ResponseHandler handler) {
        return delegate.flush(new ResponseHandler() {
            ...
            public void onError(Throwable throwable) {
                throwable = handledError(throwable);
    ...
    public CompletionStage<Void> close() {
        provider.decreaseCount(connectionInfo().serverAddress());
    ...
    private Throwable handledServiceUnavailableException(ServiceUnavailableException e) {
        routingTableHandler.onConnectionFailure(connectionInfo().serverAddress());
        return new SessionExpiredException(
                format("Server at %s is no longer available", connectionInfo().serverAddress()), e);
    }
    ...
    private Throwable handledClientException(ClientException e) {
        if (isFailureToWrite(e)) {
            ...
            switch (accessMode) {
                ...
                case WRITE -> {
                    routingTableHandler.onWriteFailure(connectionInfo().serverAddress());
                    return new SessionExpiredException(format(
                            "Server at %s no longer accepts writes",
                            connectionInfo().serverAddress()));
                }
                default -> throw new IllegalArgumentException(connectionInfo().serverAddress() + " not supported.");
            }
        }
        return e;
    }
