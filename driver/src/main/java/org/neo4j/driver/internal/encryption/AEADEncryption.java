...
public final class AEADEncryption {
    ...
    Value decrypt(AEADEncryptedProperty encryptedProperty, SecretKey key, Value aad, Provider provider)
        ...
        var gcmParameterSpec = new GCMParameterSpec(128, iv);
        var cipher = prepareCipher(DECRYPT_MODE, key, gcmParameterSpec, aadBytes, provider);
    ...
    private ValueEncoder.Encoded encode(Value value) {
        try {
            ...
            throw new PropertyEncryptionException("Failed to encode value", e);
        }
    }
    ...
    private Value decode(byte[] bytes) {
        try {
            ...
            throw new PropertyEncryptionException("Failed to decode value", e);
        }
    }
    ...
    public byte[] encodeToEncryptedBytes(AEADEncryptedProperty encryptedProperty) {
        ...
        try {
            ...
            throw new PropertyEncryptionException("Failed to encode Encrypted Structure", e);
        }
    ...
    public AEADEncryptedProperty decodeEncryptedBytes(byte[] bytes) {
        ...
        if (version != ENCODED_BYTES_VERSION) {
            throw new PropertyEncryptionException("Unsupported encrypted property version: " + version);
        }
        ...
        try {
            ...
            throw new PropertyEncryptionException("Failed to decode Encrypted Structure", e);
        }
