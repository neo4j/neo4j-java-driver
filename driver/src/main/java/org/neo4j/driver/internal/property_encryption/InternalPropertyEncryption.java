...
public final class InternalPropertyEncryption extends AbstractPropertyEncryption<EncapsulatedKeyManager>
    ...
    @Override
    public byte[] encryptToBytes(PropertyEncryptionRequest encryptRequest) {
        var encryptObservation = observationProvider.encryptToBytes(PropertyEncryption.class);
        return observe(
                encryptObservation,
                () -> encryptToBytesAsync(encryptRequest).toCompletableFuture().join());
    }

    @Override
    public Value decrypt(PropertyDecryptionRequest decryptRequest) {
        var decryptObservation = observationProvider.decrypt(PropertyEncryption.class);
        return observe(
                decryptObservation,
                () -> decryptAsync(decryptRequest).toCompletableFuture().join());
    }
