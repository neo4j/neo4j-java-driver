...
final class InternalReactiveEncapsulatedKeyManager extends AbstractEncapsulatedKeyManager
    ...
    public Publisher<EncapsulatedKey> create(String alias, KeyEncapsulationOptions encapsulationOptions) {
        return Mono.fromCompletionStage(() -> createAsync(alias, encapsulationOptions));
    }

    @Override
    public Publisher<EncapsulatedKey> findByAlias(String alias) {
        return Mono.fromCompletionStage(() -> findByAliasAsync(alias));
    }

    @Override
    public Publisher<Void> updateAliasById(String id, String alias) {
        return Mono.fromCompletionStage(() -> updateAliasByIdAsync(id, alias));
    }

    @Override
    public Publisher<Void> deleteById(String id) {
        return Mono.fromCompletionStage(() -> deleteByIdAsync(id));
    }
}
