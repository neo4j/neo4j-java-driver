...
public class InternalReactiveSession extends AbstractReactiveSession<ReactiveTransaction> implements ReactiveSession, BaseReactiveQueryRunner
{
    ...
    {
        return runTransaction( AccessMode.READ, tx -> callback.execute( new DelegatingReactiveTransactionContext( tx ) ), config );
    }

    @Override
    public <T> Publisher<T> executeWrite( ReactiveTransactionCallback<? extends Publisher<T>> callback, TransactionConfig config )
    {
        return runTransaction( AccessMode.WRITE, tx -> callback.execute( new DelegatingReactiveTransactionContext( tx ) ), config );
    }
