...
public class TemporalAuthTokenManager implements AuthTokenManager {
    private final ReadWriteLock lock = new ReentrantReadWriteLock();
    private final Supplier<CompletionStage<TemporalAuthData>> freshTokenSupplier;
    private final Clock clock;
    private CompletableFuture<AuthToken> tokenFuture;
    private TemporalAuthData token;

    public TemporalAuthTokenManager(Supplier<CompletionStage<TemporalAuthData>> freshTokenSupplier, Clock clock) {
    ...
    private void handleUpstreamResult(TemporalAuthData temporalAuthData, Throwable throwable) {
        if (throwable != null) {
            ...
            if (isValid(temporalAuthData)) {
                ...
                var currentTokenFuture = executeWithLock(lock.writeLock(), () -> {
                    token = temporalAuthData;
                    return tokenFuture;
                });
                currentTokenFuture.complete(temporalAuthData.authToken());
            }
        }
    }
    ...
    private CompletionStage<TemporalAuthData> getFromUpstream() {
        CompletionStage<TemporalAuthData> upstreamStage;
        try {
    ...
    private boolean isValid(TemporalAuthData token) {
    ...
    public record InternalTemporalAuthData(AuthToken authToken, long expirationTimestamp)
            implements AuthTokenManagers.TemporalAuthData {}
}
