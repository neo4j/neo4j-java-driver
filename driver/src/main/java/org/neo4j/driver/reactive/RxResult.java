...
public interface RxResult {
     ...
     * <p>
     * When this publisher is {@linkplain Flow.Publisher#subscribe(Flow.Subscriber) subscribed}, the query is sent to the server and executed.
     ...
     */
    Flow.Publisher<List<String>> keys();

    /**
     * Returns a cold unicast publisher of records.
     * <p>
     * When the record publisher is {@linkplain Flow.Publisher#subscribe(Flow.Subscriber) subscribed},
     ...
     * <p>
     * Therefore the {@link Flow.Subscriber} of this record publisher shall wait for the termination signal (complete or error)
     ...
     * When the execution is finished, the {@link Flow.Subscriber} will be notified with a termination signal (complete or error).
     * <p>
     ...
     * <p>
     * This publisher can only be subscribed by one {@link Flow.Subscriber} once.
     * <p>
     ...
     */
    Flow.Publisher<Record> records();

    /**
     * Returns a cold publisher of result summary which arrives after all records.
     * <p>
     * {@linkplain Flow.Publisher#subscribe(Flow.Subscriber) Subscribing} the summary publisher results in the execution of the query followed by the result summary being returned.
     ...
     */
    Flow.Publisher<ResultSummary> consume();
