...
class ReactivePropertyEncryptionIT extends AbstractPropertyEncryptionIT<ReactivePropertyEncryption> {
    @Override
    ...
    protected byte[] encryptToBytes(PropertyEncryptionRequest request) {
        return Flux.from(JdkFlowAdapter.flowPublisherToFlux(propertyEncryption.encryptToBytes(request)))
                .blockFirst();
    }

    @Override
    protected Value decrypt(PropertyDecryptionRequest request) {
        return Flux.from(JdkFlowAdapter.flowPublisherToFlux(propertyEncryption.decrypt(request)))
                .blockFirst();
    }

    @Override
    protected void createKey() {
        Flux.from(JdkFlowAdapter.flowPublisherToFlux(
