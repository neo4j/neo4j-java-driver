...
public class ClusterDockerProcess extends DockerProcess
{
    ...
    public void stop( String name )
    {
        try
        {
            process.getOutputStream().write( String.format( "stop %s\n", name ).getBytes() );
            process.getOutputStream().flush();
            waitForCommandReturn();
        }
        catch ( IOException e )
        {
            throw new CommandLineException( "Failed to stop a cluster member", e );
        }
    ...
    public void startAll()
    {
        try
        {
            process.getOutputStream().write( "start all\n".getBytes() );
            process.getOutputStream().flush();
            waitForCommandReturn();
        }
        catch ( IOException e )
        {
            throw new CommandLineException( "Failed to restart all stopped cluster member", e );
        }
    }
}
