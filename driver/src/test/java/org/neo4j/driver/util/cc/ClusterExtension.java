...
public class ClusterExtension implements BeforeAllCallback, AfterEachCallback, AfterAllCallback {
    private static final Path CLUSTER_DIR =
            Paths.get(TARGET_DIR, "test-cluster").toAbsolutePath();
    ...
    public void beforeAll(ExtensionContext context) throws Exception {
        ...
        if (!SharedCluster.exists()) {
            SharedCluster.install(
                    parseNeo4jVersion(), CORE_COUNT, READ_REPLICA_COUNT, PASSWORD, INITIAL_PORT, CLUSTER_DIR);
