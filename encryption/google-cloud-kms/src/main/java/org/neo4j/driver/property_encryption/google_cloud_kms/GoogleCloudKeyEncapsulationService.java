...
public final class GoogleCloudKeyEncapsulationService implements KeyEncapsulationService {
    ...
    public CompletionStage<EncapsulationResult> encapsulate(KeyEncapsulationOptions options) {
        ...
        if (encapsulationOptions instanceof CloudKmsKeyEncapsulationOptions cloudOptions) {
            ...
            return toCompletionStage(
                    ...
                    .thenApply(resp ->
                            EncapsulationResult.of(resp.getCiphertext().toByteArray(), encapsulationOptions, key));
        } else {
    ...
    @Override
    public CompletionStage<SecretKey> decapsulate(byte[] encapsulation, Map<String, String> options) {
        if (options == null) {
            return CompletableFuture.failedStage(new ClientException("Missing options"));
        }
        var cloudOptions = CloudKmsKeyEncapsulationOptions.of(options);
