...
{
    private final TestkitState testkitState = new TestkitState();
    ...
    public boolean process() throws JsonProcessingException
    {
            ...
            else if ( currentLine.equals( "#request end" ) )
            {
                ...
                catch ( Exception e )
                {
                    if ( e instanceof Neo4jException )
                    {
                        ...
                        writeResponse( TestkitErrorResponse.builder().errorMessage( "Something blew up" + e ).build() );
    ...
    public void processRequest( String request ) throws JsonProcessingException
    {
        ...
        try
        {
            ...
            //todo missing TransactionRun, RetryableNegative, SessionReadTransaction,
    ...
    private void writeResponse( TestkitResponse response ) throws JsonProcessingException
    {
        //System.out.println( "response = " + objectMapper.writeValueAsString( response ) );

        write( "#response begin\n" );
        write( objectMapper.writeValueAsString( response ) + "\n" );
        write( "#response end\n" );
        flush();
    }
}
